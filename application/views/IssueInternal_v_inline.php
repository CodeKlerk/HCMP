...
//user1.attr("value","");
user1.attr("id",unit_size_id);
>>>>>>> 6944d2512953fb959e883caec006ee4f052800dd
...
//	alert(therow);
            ...
            $('#avlb_Stock_' + number).val(batch_split[1]);
            $('#avlb_hide').val(batch_split[1]);
            var prevrowval = $('#avlb_Stock_' + prevrow).val();
            var val = $('#desc_' + therow).val();
            get_similar_comodities(val);

            /*
             if (prevrowval>0){
             
             //  alert (prevrowval);
             $('#avlb_Stock_' + number).val(prevrowval);
             $('#avlb_hide').val(prevrowval);   
             }
             */
...
// test
    function get_similar_comodities(desc) {
        var descarr = [];
        descarr.push(desc);
       alert(desc);
        $table = $('#mytable');
...
//        alert(numItems);
        var cloned_object = $table.find('tr:last');
        var issue_row = cloned_object.attr("issue_row");
        var therow = parseInt(issue_row);


        var rows = $('tbl').children('tr');
        theval = rows.find('#batchNo_'+therow).val();
  //       alert(theval);
    ...
    <table class="data-table" id="mytable" width="100%" style="background: rgb(218, 218, 218);" >
        ...
        <tbody>
            <tr issue_row="1">
                ...
                <td>
                    <select class="dropdownsize" id="desc_1" name="desc">
...
quantity.attr("value","")
servicepoint.attr("id",Servicepoint_id);
date.attr("value", today_full_date);
date.attr("id",datepicker_id);
...
//						$('input:text[name=avlb_Stock]').val(remainder);
  ...
  function refreshDatePickers() {
	 ...
	 <form action="http://localhost/HCMP/Issues_main/Insert" method="post" accept-charset="utf-8" name="myform" id="myform">
<table class="data-table" id="mytable" width="100%" style="background: rgb(218, 218, 218);" >
	<thead>
	<tr>
	<th><b>S11 No</b></th>
...
<input class="avlb_Stock" id="avlb_Stock_1" name="avlb_Stock" disabled="disabled">	</td>
	...
	<td width="70"><input type="text" class="quantity" name="qty" id="qty_1" value=""></td>
	<td width="130">
            <?php 
